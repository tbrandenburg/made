...
class AgentCLI(ABC):
    ...
    def list_sessions_legacy(self, cwd: Path | None) -> subprocess.CompletedProcess:
        """Legacy method for backward compatibility."""
        raise NotImplementedError

    def list_agents_legacy(self) -> subprocess.CompletedProcess:
        """Legacy method for backward compatibility."""
        raise NotImplementedError
...
class OpenCodeAgentCLI(AgentCLI):
    ...
    def _parse_opencode_output(self, stdout: str) -> tuple[str | None, list[ResponsePart]]:
        ...
        for index, part in enumerate(parts):
            ...
            if kind == "text":
                part_type = "final" if text_indices and index == text_indices[-1] else "thinking"
            else:
                part_type = "tool"
    ...
    def _resolve_part_timestamp(self, part: dict[str, object], fallback: int | None) -> int | None:
    ...
    def run_agent(
        ...
        try:
            ...
            process = self.start_run(command, cwd)
            stdout, stderr = process.communicate(input=message)

            if process.returncode == 0:
                parsed_session_id, response_parts = self._parse_opencode_output(stdout or "")
                return RunResult(
                    success=True,
        ...
        try:
            with tempfile.NamedTemporaryFile(mode="w+", encoding="utf-8", delete=True) as tmp:
                ...
                if result.returncode != 0:
                    error_message = (result.stderr or "").strip() or "Failed to export session history"
                    return ExportResult(
                        success=False,
                        session_id=session_id,
                        messages=[],
    ...
    def list_sessions(self, cwd: Path | None) -> SessionListResult:
        ...
        try:
            ...
            if result.returncode != 0:
                error_message = (result.stderr or "").strip() or "Failed to list sessions"
            ...
            sessions = self._parse_session_table(result.stdout or "", 50)  # Default limit
            return SessionListResult(
                success=True,
                sessions=sessions
            )
