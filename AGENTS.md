...
- 2026-07-18: Do not assume parallel CLI implementations share parser helpers. Before wiring a success path to an existing parser, verify the target class defines it and test reasoning, text, and tool JSON parts.
...
- 2026-06-30: `useState` with object values bypasses React's Object.is bailout. If a component passes an inline object literal as a prop/callback (recreated every render) and the hook calls `setState({ ... })` with a new object — even when logically unchanged — React re-renders, which recreates the inline object, which triggers the effect again: infinite loop. Prevention: use functional update form `setState(prev => prev.field === X ? prev : newValue)` everywhere object state is set. This returns the same reference when unchanged, enabling React's bailout.
- 2026-06-30: When moving a class to a new module, grep ALL test directories (unit AND integration) for the old import before committing. `make qa-quick` only covers `tests/unit/` — integration tests are invisible to the local pre-push gate and can silently break CI. Prevention: after any module rename/move, run `grep -r "from <old_module> import" tests/` to find all stale imports across every test subdirectory.
- 2026-07-18: Full backend pytest includes integration tests that invoke external agent CLIs and can fail or exceed a short local timeout even when unit/system coverage is green. Prevention: run the complete suite with a realistic timeout, then report external-CLI failures separately from scoped regression results.
- 2026-08-03: Adopting a third-party PyPI dependency (e.g. `flowsh-cli`) can silently force a `requires-python` bump on the whole package (`uv sync` fails to resolve otherwise) — this is easy to miss when only checking `make qa-quick-backend` locally on a newer interpreter. Prevention: after adding any new backend dependency, check its own `requires-python`/classifiers before pinning a version, and grep CI workflow files for a pinned Python version to confirm compatibility before merging.
- 2026-08-03: `uv export --frozen | pip-audit -r /dev/stdin` fails with uv 0.9.9 because `uv export` emits an editable requirement (`-e .`) for the local package alongside hash-pinned dependencies, and pip refuses editable+`--require-hashes` combinations. Prevention: always add `--no-hashes` to `uv export` when piping into `pip-audit` (`uv export --frozen --no-hashes | uv run --with pip-audit -- pip-audit -r /dev/stdin`); verify this on an unmodified branch first before assuming a dependency change caused an audit-command failure.
- 2026-08-03: `npm audit fix --force`'s suggested version can be a downgrade relative to a version bump made earlier for a different CVE, silently reintroducing the first vulnerability while "fixing" a newer one (seen with `react-router-dom`: audit suggested `7.11.0` to fix a new advisory, but `7.18.2` was required to fix two prior CVEs). Prevention: before applying any `npm audit fix`/`--force` suggestion, diff the proposed version against versions required by other currently-tracked security fixes/issues, not just the one advisory being addressed.
- 2026-08-05: Git sets `GIT_DIR`/`GIT_WORK_TREE`/`GIT_INDEX_FILE` in the environment when invoking hooks (e.g. `pre-push`). If a hook script runs test suites without unsetting these, any subprocess `git init`/`git commit` call inside a test fixture (even one using `cwd=tmp_path`) ignores `cwd` and mutates the *real* repository instead — observed as a stray commit, a truncated `README.md`, and an invalid `core.bare=true` config that broke git entirely. Prevention: any hook or CI script that shells out to test suites which themselves shell out to `git` must `unset GIT_DIR GIT_WORK_TREE GIT_INDEX_FILE GIT_OBJECT_DIRECTORY GIT_ALTERNATE_OBJECT_DIRECTORIES` first. If this happens, recover immediately with `git reflog` + `git reset --hard <last known good commit>` and manually strip any injected `bare`/`worktree` lines from `.git/config` before doing anything else.
